...
const TRAINING_LOG_CHANNEL_ID = '1499802939441807442';

const RETIREMENT_LOG_CHANNEL_ID = '1508481479519244500';
const RETIREMENT_LOG_PERMISSION_ROLE_IDS = ['1497415247860994058'];

const DISCORD_CHECK_LOG_CHANNEL_ID = '1505260997668114573';
const DISCORD_CHECK_PERMISSION_ROLE_ID = '1496040532734902313';
...
const TICKET_TYPES = {
  management: {
    ...
    supportRoleIds: ['1497016268300226661']
  },
  general: {
    label: 'General Support',
    value: 'general',
    categoryId: '1497012918158491728',
    supportRoleIds: ['1497016357491970129']
  },
  community: {
    label: 'Community Support',
    value: 'community',
    categoryId: '1497012968364183562',
    supportRoleIds: ['1497016398353006592']
  },
  departments: {
    label: 'Departments',
    value: 'departments',
    categoryId: '1508130057732751520',
    supportRoleIds: [
      '1508118125655363635',
      '1508120119841390662',
      '1508118071695642705'
...
const SUPPORT_ROLE_IDS = [
  ...new Set(
    Object.values(TICKET_TYPES).flatMap(ticket =>
      ticket.supportRoleIds || [ticket.supportRoleId]
    )
  )
];
  ...
  const commands = [
    ...
    },
    {
      ...
      options: [
        ...
        {
          name: 'log',
          description: 'Log a staff training result.',
          type: 2,
          options: [
            {
              name: 'phase1',
              description: 'Log a Phase 1 training result.',
              type: 1,
              options: [
                { name: 'trainee', description: 'User being trained.', type: 6, required: true },
                { name: 'knowledge_of_punishments', description: 'Knowledge of punishments score out of 10.', type: 4, required: true, min_value: 0, max_value: 10 },
                { name: 'spag', description: 'SPaG score out of 10.', type: 4, required: true, min_value: 0, max_value: 10 },
                {
                  name: 'result',
            ...
            {
              name: 'phase2',
              description: 'Log a Phase 2 ride-along result.',
              type: 1,
              options: [
                { name: 'trainee', description: 'User being trained.', type: 6, required: true },
                { name: 'speed', description: 'Speed score out of 10.', type: 4, required: true, min_value: 0, max_value: 10 },
                { name: 'driving', description: 'Driving score out of 10.', type: 4, required: true, min_value: 0, max_value: 10 },
                { name: 'mod_calls', description: 'Mod calls score out of 10.', type: 4, required: true, min_value: 0, max_value: 10 },
                {
                  name: 'result',
                  description: 'Training result.',
                  type: 3,
    ...
    {
      name: 'log',
      description: 'Log staff actions.',
      options: [
        {
          name: 'retirement',
          description: 'Log a staff retirement.',
          type: 1,
          options: [
            { name: 'staff_member', description: 'The retired staff member.', type: 6, required: true },
            { name: 'date_of_retirement', description: 'Date of retirement.', type: 3, required: true },
            { name: 'reason', description: 'Reason for retirement.', type: 3, required: true }
          ]
        },
        {
          name: 'discord',
          description: 'Discord log commands.',
          type: 2,
          options: [
            {
              name: 'check',
              description: 'Log a Discord check.',
              type: 1,
              options: [
                { name: 'roblox_username', description: 'Roblox username.', type: 3, required: true },
                { name: 'discord_username', description: 'Discord username.', type: 3, required: true },
                { name: 'discord_join_link', description: 'Discord join link.', type: 3, required: true }
              ]
            }
          ]
        }
...
async function handlePromotionCommand(interaction) {
  ...
  const promotionEmbeds = buildPromotionEmbeds({
    targetUser,
    newRankRole,
    notes,
    issuer: interaction.user
  ...
  try {
    await targetUser.send({
      content: `Congratulations! You have been promoted in Minnesota State Roleplay Community.`,
      embeds: promotionEmbeds
    });
  } catch {
    dmSent = false;
  ...
  await interaction.reply({
    content: dmSent
      ? 'Promotion posted and the user was DM’d.'
      : 'Promotion posted, but I could not DM the user.',
    ephemeral: true
  });
}
...
const LOG_BOTTOM_IMAGE_URL =
  'https://media.discordapp.net/attachments/1393378962364956783/1393379706010603561/minstate.png?ex=6a1589b9&is=6a143839&hm=08095b00b9a41ef2edc1680a4bc2bfee7737028cc1c2cdddc912e55522492738&=&format=webp&quality=lossless&width=1860&height=94';

function buildTrainingLogComponents({ phase, trainedUserId, trainedById, result, knowledgeOfPunishments, spag, speed, driving, modCalls }) {
  const content = phase === 'phase1'
    ? '## Phase: 1\n\n' +
      `Trainee: <@${trainedUserId}>\n\n` +
      `Knowledge of punishments: ${knowledgeOfPunishments}/10\n` +
      `SPaG: ${spag}/10\n` +
      `Result: ${result}\n\n` +
      `-# Trained by: <@${trainedById}>`
    : '## Phase: 2\n\n' +
      `Trainee: <@${trainedUserId}>\n\n` +
      `Speed: ${speed}/10\n` +
      `Driving: ${driving}/10\n` +
      `Mod calls: ${modCalls}/10\n` +
      `Result: ${result}\n\n` +
      `-# Trained by: <@${trainedById}>`;
  ...
  return [
    {
      ...
      components: [
        { type: 10, content },
        { type: 12, items: [{ media: { url: LOG_BOTTOM_IMAGE_URL.replace('width=1860&height=94', 'width=1607&height=81') } }] }
      ],
      accent_color: 11611694
    }
  ];
}

async function handleTrainingLogCommand(interaction) {
  if (!hasRole(interaction.member, TRAINER_ROLE_ID)) {
    await interaction.reply({ content: 'You do not have permission to log training.', ephemeral: true });
    return;
  }

  const phase = interaction.options.getSubcommand();
  const trainedUser = interaction.options.getUser('trainee', true);
  const result = interaction.options.getString('result', true);
  const channel = await client.channels.fetch(TRAINING_LOG_CHANNEL_ID).catch(() => null);

  if (!channel || !channel.isTextBased()) {
    await interaction.reply({ content: 'Training log channel was not found.', ephemeral: true });
    return;
  }

  const components = phase === 'phase1'
    ? buildTrainingLogComponents({
        phase,
        trainedUserId: trainedUser.id,
        trainedById: interaction.user.id,
        knowledgeOfPunishments: interaction.options.getInteger('knowledge_of_punishments', true),
        spag: interaction.options.getInteger('spag', true),
        result
      })
    : buildTrainingLogComponents({
        phase,
        trainedUserId: trainedUser.id,
        trainedById: interaction.user.id,
        speed: interaction.options.getInteger('speed', true),
        driving: interaction.options.getInteger('driving', true),
        modCalls: interaction.options.getInteger('mod_calls', true),
        result
      });

  await channel.send({ flags: IS_COMPONENTS_V2, components });
  await interaction.reply({ content: `Training log sent to <#${TRAINING_LOG_CHANNEL_ID}>.`, ephemeral: true });
}

function buildRetirementLogComponents({ staffMemberId, loggedById, dateOfRetirement, reason }) {
  return [
    {
      type: 17,
      components: [
        {
          ...
          content:
            '## Retirement Log\n' +
            `**\`Retired Staff Member:\`** <@${staffMemberId}>\n` +
            `**\`Logged by:\`** <@${loggedById}>\n` +
            `**\`Date of Retirement:\`** ${dateOfRetirement}\n` +
            `**\`Reason:\`** ${reason}`
        },
        { type: 12, items: [{ media: { url: LOG_BOTTOM_IMAGE_URL } }] }
      ],
      accent_color: 13840422
    }
  ];
}

async function handleLogRetirementCommand(interaction) {
  if (!RETIREMENT_LOG_PERMISSION_ROLE_IDS.some(roleId => hasRole(interaction.member, roleId))) {
    await interaction.reply({ content: 'You do not have permission to log retirements.', ephemeral: true });
    return;
  }

  const staffMember = interaction.options.getUser('staff_member', true);
  const dateOfRetirement = interaction.options.getString('date_of_retirement', true);
  const reason = interaction.options.getString('reason', true);
  const channel = await client.channels.fetch(RETIREMENT_LOG_CHANNEL_ID).catch(() => null);

  if (!channel || !channel.isTextBased()) {
    await interaction.reply({ content: 'Retirement log channel was not found.', ephemeral: true });
    return;
  }

  await channel.send({
    flags: IS_COMPONENTS_V2,
    components: buildRetirementLogComponents({
  ...
  await interaction.reply({ content: `Retirement log sent to <#${RETIREMENT_LOG_CHANNEL_ID}>.`, ephemeral: true });
}

function buildDiscordCheckLogComponents({ robloxUsername, discordUsername, discordJoinLink, loggedById }) {
  return [
    {
      type: 17,
      components: [
        {
          ...
          content:
            ...
            `Discord Join Link: ${discordJoinLink}\n` +
            `-# Logged by: <@${loggedById}>`
        },
        { type: 12, items: [{ media: { url: LOG_BOTTOM_IMAGE_URL } }] }
      ],
...
async function handleLogDiscordCheckCommand(interaction) {
  if (!hasRole(interaction.member, DISCORD_CHECK_PERMISSION_ROLE_ID)) {
    await interaction.reply({ content: 'You do not have permission to log Discord checks.', ephemeral: true });
    return;
  }

  const players = await getCurrentERLCPlayerCount();

  if (players === null) {
    await interaction.reply({ content: 'I could not check the in-game player count right now. Please try again later.', ephemeral: true });
    return;
  }

  if (players <= 0) {
    await interaction.reply({ content: 'This command cannot be used because there are 0 people in-game.', ephemeral: true });
    return;
  }

  const channel = await client.channels.fetch(DISCORD_CHECK_LOG_CHANNEL_ID).catch(() => null);

  if (!channel || !channel.isTextBased()) {
    await interaction.reply({ content: 'Discord check log channel was not found.', ephemeral: true });
    return;
  }

  await channel.send({
    flags: IS_COMPONENTS_V2,
    components: buildDiscordCheckLogComponents({
      ...
      discordJoinLink: interaction.options.getString('discord_join_link', true),
      loggedById: interaction.user.id
    })
  });

  await interaction.reply({ content: `Discord check log sent to <#${DISCORD_CHECK_LOG_CHANNEL_ID}>.`, ephemeral: true });
}
...
async function handleTrainingRequest(interaction) {
  if (!(await requireMinimumPlayers(interaction, 5))) return;
      ...
      components: [
        {
          ...
          options: [
            ...
            { label: 'Departments', value: 'departments', emoji: { name: 'departments', id: '1499498894390722821' } }
          ]
        }
      ]
    }
...
async function getCurrentERLCPlayerCount() {
  try {
    const data = await fetchERLCServerStats();
    return Number(data.CurrentPlayers ?? 0);
  } catch (error) {
    console.error('Failed to fetch ERLC player count:', error);
    return null;
  }
}

async function requireMinimumPlayers(interaction, minimumPlayers) {
  const players = await getCurrentERLCPlayerCount();

  if (players === null) {
    await interaction.reply({
      content: 'I could not check the in-game player count right now. Please try again later.',
      ephemeral: true
    });
    return false;
  }
  ...
  if (players < minimumPlayers) {
    await interaction.reply({
      content: `This command cannot be used unless there are at least ${minimumPlayers} players in-game. Current players: ${players}.`,
      ephemeral: true
    });
    return false;
  }
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'training') {
        ...
        if (subcommandGroup === 'log') return await handleTrainingLogCommand(interaction);
      ...
      if (interaction.commandName === 'log') {
        ...
        if (subcommand === 'retirement') return await handleLogRetirementCommand(interaction);
        if (subcommandGroup === 'discord' && subcommand === 'check') return await handleLogDiscordCheckCommand(interaction);
      }
    }
