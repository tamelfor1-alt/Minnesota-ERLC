  ...
  return [
    {
      ...
      components: [
        ...
        {
          type: 12,
          items: [{ media: { url: GIVEAWAY_BOTTOM_IMAGE } }]
        }
      ],
      spoiler: false,
...
async function handleGiveawayHostCommand(interaction) {
  if (!hasGiveawayPermission(interaction.member)) {
    await interaction.reply({
      content: 'You do not have permission to host giveaways.',
      ephemeral: true
    });
    return;
  }
...
async function handleTrainingAccept(interaction) {
  if (!hasRole(interaction.member, TRAINER_ROLE_ID)) {
    await interaction.reply({
      content: 'Only staff trainers can accept training requests.',
      ephemeral: true
    });
    return;
  }
...
function buildShutdownEmbed(message) {
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Shutdown by: ${message.member.displayName}`, iconURL: message.author.displayAvatarURL() });
}
...
client.on(Events.MessageCreate, async message => {
  try {
    ...
    if (!message.member.roles.cache.has(SESSION_MANAGER_ROLE_ID)) return;

    if (content === '.shutdown') {
      await message.channel.send({ embeds: [buildShutdownEmbed(message)], components: [buildNotifierButtonRow()] });
      await message.delete().catch(() => {});
      return;
    }

    if (content === '.startup') {
      await message.channel.send({ content: `<@&${SESSION_NOTIFIER_ROLE_ID}>`, embeds: [buildStartupEmbed()], components: [buildNotifierButtonRow()] });
      await message.delete().catch(() => {});
      return;
    }

    if (content === '.startupvote') {
      const existingVote = activeStartupVotes.get(message.guild.id);

      if (existingVote && !existingVote.ended) {
        const stillExists = await doesVoteMessageStillExist(existingVote);
        if (!stillExists) activeStartupVotes.delete(message.guild.id);
        else {
          const notice = await message.channel.send('There is already an active startup vote.');
          await message.delete().catch(() => {});
    ...
    if (content === '.boost') {
      await message.channel.send({ embeds: [buildBoostEmbed()], components: [buildJoinButtonRow()] });
      await message.delete().catch(() => {});
    }
  } catch (error) {
